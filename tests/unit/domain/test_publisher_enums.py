...
def test_redis_streams_are_correct_strings():
...
def test_redis_streams_enum_has_three_allowed_components():
    assert len(RedisStreams) == 3


def test_redis_streams_inherit_from_enum_class():
    assert isinstance(RedisStreams.NOTIFICATIONS, Enum)


def test_enum_comparison_works_as_expected():
...
def test_redis_streams_raises_value_error_for_invalid_string():
    with pytest.raises(ValueError):
        RedisStreams("notifications_invalid")


def test_redis_streams_raises_value_error_for_integer_value():
    with pytest.raises(ValueError):
        RedisStreams(123)
